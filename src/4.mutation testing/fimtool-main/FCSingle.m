...
% FCSingle(Configuration file, Output Directory, Fault_list)
...
function FCSingle(Faultinjector_config, result_path, Fault_enable_list)
...
% First disable all fault blocks (if any of them is activated)
for g = 1 : height(newtable)
    % First read the details of the corresponding fault injector block
    FIBName = cell2mat(table2array(newtable(g,2)));
    Parentblock = table2array(newtable(g,3));
    hh = Parentblock{1,1};
    Fullpath = strcat(hh, '/', FIBName);
    
    % turn off the fault injector block
    % disp([ 'Turning off the block ', FIBName, ' .' ]);
    set_param(Fullpath, 'FIEnableFlag', "off");
    set_param(strcat(Fullpath, '/Enable_flag'), 'Value', "0");
...
% Then enable the selected fault blocks based on fault enable list
for p = 1 : height(FEL)
