...
for i = 1:length(mutants_list_)
    if contains(mutants_list_(i).name, ".slx") && ~contains(mutants_list_(i).name, ".autosave") && ~contains(mutants_list_(i).name, "c.slx")
...
writematrix([first_row; [mutants_list; second_column; killed_list; third_column]'], "TSFIM_req.csv");
