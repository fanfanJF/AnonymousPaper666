...
for i = 1:length(mutants_list_)
    if contains(mutants_list_(i).name, ".slx") && ~contains(mutants_list_(i).name, ".autosave") && ~contains(mutants_list_(i).name, "slxc")
...
index_to_remove = find(strcmp(mutants_list, original_model));
mutants_list(index_to_remove) = [];
...
while ~isempty(mutants_index) & tc_cnt<=20 %loop on TSmain
    ...
    for i = 1:length(fitness)
        if fitness_org{i} >= 0 && fitness{i} < 0
            found = true;
            break;
        end
    end
...
writematrix([first_row; [mutants_list; second_column; killed_list; third_column]'], "TSbert_req.csv");
