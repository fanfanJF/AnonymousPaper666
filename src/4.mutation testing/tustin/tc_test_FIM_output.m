...
while ~isempty(mutants_index) & tc_cnt<=10 %loop on TSmain
    ...
    if ~isequal(y1_org, y1) %mutant is killed with input 1
    ...
    if ~isequal(y2_org, y2) %mutant is killed with input 2
    ...
    if ~isequal(y3_org, y3) %mutant is killed with input 3
    ...
    if ~isequal(y4_org, y4) %mutant is killed with input 4
...
writematrix([first_row; [mutants_list; second_column; killed_list; third_column]'], "TSFIM_output.csv");
